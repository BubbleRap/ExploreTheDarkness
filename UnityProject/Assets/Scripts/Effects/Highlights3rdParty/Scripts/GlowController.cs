...
public class GlowController : MonoBehaviour
{
	...
	private List<Renderer> _glowableObjects = new List<Renderer> ();
	...
	/// </summary>
	public static void RegisterObject(Renderer glowObj)
	{
		_instance.m_currentColor = Color.black;
		_instance.m_targetColor = _instance.m_highlightColor;
		_instance._glowableObjects.Add(glowObj);
	}

	public static void ResetAllObjects()
	{
		_instance.m_targetColor = Color.black;
		_instance._glowableObjects.Clear();
	}
	...
	private void RebuildCommandBuffer()
	{
		...
		foreach(var pair in _glowableObjects)
		{
			_commandBuffer.SetGlobalColor(_glowColorID, m_currentColor);
			_commandBuffer.DrawRenderer(pair, _glowMat);
		}
	...
	private void Update()
	{
		m_currentColor = Color.Lerp (m_currentColor, m_targetColor, Time.deltaTime * 2f);
